...
function CalcFTC(){
  ...
  //Find the indexes the user inputted on our arrays
  var lowerIndex = xValuesFTC.indexOf(x1);
  var upperIndex = xValuesFTC.indexOf(x2);

  var ftcValue = 0;
  //Calculate the areas of tiny rectangles 0.001 units wide and accumulate them
  for(var i = lowerIndex; i <=upperIndex; i++){
    ftcValue += yValuesFTC[i]*XSTEP
  }

  ftcValue = ftcValue.toFixed(3)
